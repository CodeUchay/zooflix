...
import { RiMovieFill, RiShoppingCart2Fill } from "react-icons/ri";
    ...
    <div className="container p-5 ">
      <div className="flex  gap-1 justify-between flex-row lg:px-6">
        ...
        <div className="flex gap-4 ">
          ...
          <span className="hidden lg:block lg:text-2xl ">
            { user && (<p>Welcome, {user.firstName + " " + user.lastName}</p>)}
          </span>
          {/* Cart container */}
          <div
            to="#"
            className="bg-orange-400 hover:bg-orange-500 px-2 py-1 hover:shadow-lg rounded-full flex justify-between items-center  cursor-pointer"
          >
            { user && (<p>{user.firstName[0].toUpperCase() + user.lastName[0].toUpperCase()}</p>)}
      ...
      >
        ...
        <nav>
          <ul className="flex flex-col p-4 ">
            ...
            <li className="text-xl  py-4 px-5 flex mt-5 hover:rounded-sm">
              <button onClick={(e) => logout()}>
                <Link
                  to="/"
                  className="flex justify-between items-center p-2 cursor-pointer  hover:bg-red-500 border border-gray-800 rounded"
                >
                  Logout
                </Link>
              </button>
